window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function updateEntryLabels() {
      ...
      const periodLabel = entry.querySelector('[data-stat-key="todayPeriod"] .dshBalanceStatLabel');
      if (spendLabel) spendLabel.textContent = t("todaySpend");
      if (tokenLabel) tokenLabel.textContent = t("todayTokens");
      if (periodLabel) periodLabel.textContent = t("todayPeriod");
      syncPeriodBadge();
      entry.setAttribute("aria-label", t("entryAriaLabel"));
    }
    /** Sync the DeepSeek peak/off-peak badge shown in the sidebar pill, honoring the
     *  current language. The period flips at 09:00 / 12:00 / 14:00 / 18:00, so this is
     *  called on every refresh.
     *  @param root Optional entry element already in hand. When omitted, queries document.
     *              Passing it avoids the miss that happens right after createEntry()
     *              builds the nodes but before they are inserted into the document. */
    function syncPeriodBadge(root) {
      const scope = root || document;
      const badge = scope.querySelector('[data-stat-key="todayPeriod"] .dshBalancePeriodBadge');
      if (!badge) return;
      const period = currentDeepseekPeriod();
      const next = t(period + "Period");
      if (badge.textContent !== next) badge.textContent = next;
      if (badge.dataset.period !== period) badge.dataset.period = period;
    }
    ...
    function createEntry(variant) {
      ...
      entry.setAttribute("aria-label", t("entryAriaLabel"));
      // 4th column: DeepSeek peak/off-peak period. Reuses currentDeepseekPeriod() for the
      // decision and the .dshBalancePeriodBadge data-period palette for the color. The label
      // stays neutral grey and only the value is tinted, so the sidebar does not end up with
      // a heavy red/green block.
      const periodSpan =
        '<span class="dshBalancePeriodBadge" data-period="' + currentDeepseekPeriod() + '" ' +
        'style="font-size:12px;padding:0;background:none;font-variant-numeric:tabular-nums"></span>';
      entry.innerHTML =
        ...
        statHtml(t("todayPeriod"), periodSpan, "todayPeriod");
      // Fill the period text immediately: the nodes are not in the document yet, and
      // updateEntryLabels() may already have run once before createEntry(), so without
      // this the period value would render blank.
      syncPeriodBadge(entry);
      return entry;
    }
